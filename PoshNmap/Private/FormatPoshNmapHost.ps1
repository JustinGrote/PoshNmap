...
function FormatPoshNmapHost {
    ...
    process {
        ...
        $entry = [ordered]@{
            ...
            Hostname   = $null
            Status     = ($hostnode.status.state.Trim() | Where-Object length -ge 2)
            FQDNs      = $hostnode.hostnames.hostname.name | Select-Object -Unique
            FDQN       = $null
            IPv4       = $null
            IPv6       = $null
            MAC        = $null
            ...
            OpenPorts  = $hostnode.ports.port | Measure-Object | ForEach-Object count
        }
        $entry.FQDN = $entry.FQDNs | Select-Object -first 1
        $entry.Hostname = $entry.FQDN -replace '^(\w+)\..*$', '$1'
        FormatStringOut -InputObject $entry.Ports { $this.ports | Measure-Object | ForEach-Object count }
        ...
        $CertainOS = $hostnode.os.osmatch | Where-Object { $_.accuracy -eq 100 } | Select-Object -first 1
        if ($CertainOS) { $Entry.OS = $certainOS.name; $Entry.OSDetail = $certainOS } else { $Entry.OS = $null }
        $entry.BestGuessOS = ($hostnode.os.osmatch | Select-Object -first 1).name
        $entry.BestGuessOSPercent = ($hostnode.os.osmatch | Select-Object -first 1).accuracy
        $entry.OSGuesses = $hostnode.os.osmatch
        if (@($entry.OSGuesses).count -lt 1) { $entry.OS = $null }

        if ($null = $hostnode.hostscript) {
